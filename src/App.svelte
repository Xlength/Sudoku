<script>
	...
	let unsubscribe = null;
		function sudokuWikiBdToSencode(bd) {
	if (!bd) return null;

	// 只保留数字/点，兼容有人复制时带杂字符
	const cleaned = bd.trim().replace(/[^0-9.]/g, "");

	if (cleaned.length !== 81) return null;

	// 你们的 puzzleImporter.toSencode() 期望的是“字符串编码”
	// 常见做法：把 '.' 当空，直接把 81 位串喂给 importer（很多项目都支持）
	// 如果你们 importer 不支持，我们再换成另一种格式（下面我也给了兜底）
	try {
		const imported = puzzleImporter.toSencode(cleaned);
		return imported?.sencode ?? null;
	} catch (e) {
		return null;
	}
	}
